...
export default component$(() => {
  ...
  const isAnimating = useSignal(false); // Track if animation is in progress
  ...
  const updateSearchState = $((open: boolean, query?: string) => {
    if (!open) {
      isAnimating.value = true; // Start fade-out animation
    }
  ...
  const handleAnimationEnd = $(() => {
    if (!isSearchOpen.value) {
      isAnimating.value = false; // End fade-out animation
    } else {
  ...
  return (
    <div class="relative">
      ...
      {/* Parent container controlled by Tailwind */}
      <div
        class={`${
          isSearchOpen.value || isAnimating.value ? 'block' : 'hidden'
        }`}
      >
        {/* Child element handles animations */}
        <div
          class={`fixed inset-0 z-50
            backdrop-blur bg-white/30 dark:bg-black/30
            transition-all duration-300
            ${isSearchOpen.value ? styles.searchOverlayEnter : styles.searchOverlayExit}`}
